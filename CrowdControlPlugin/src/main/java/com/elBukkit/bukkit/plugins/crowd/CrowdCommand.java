    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args[0].equalsIgnoreCase("add")) {
            ...
            for (int i : rules.keySet()) {
                sender.sendMessage(((Object) rules.get(i)).getClass().getSimpleName() + ", ID: " + i);
            }
            return true;
        } else if (args[0].equalsIgnoreCase("getDetailedInfo")) {
            if (args.length >= 2) {
                Rule r = plugin.ruleHandler.getRules().get(Integer.valueOf(args[1]));
            ...
            if (args.length >= 9) {
                CreatureInfo info = new CreatureInfo(Nature.valueOf(args[3]), Nature.valueOf(args[4]), Integer.parseInt(args[5]), Integer.parseInt(args[6]), Integer.parseInt(args[8]), Integer.parseInt(args[9]), Boolean.parseBoolean(args[7]), Float.parseFloat(args[10]), CreatureType.valueOf(args[1]), Boolean.parseBoolean(args[11]));
                try {
